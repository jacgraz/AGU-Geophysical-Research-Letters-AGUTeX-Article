...
%DIF DEL Grazioli_JGRA_2024.tex      Sun Dec  1 17:05:21 2024
%DIF ADD Grazioli_JGRA_2024_R1.tex   Mon Dec 16 11:01:56 2024
...
\title{Observation of \DIFdelbegin \DIFdel{falling snowflakes }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend orientation  \DIFaddbegin \DIFadd{of snow hydrometeors }\DIFaddend at sheltered and unsheltered sites}
...
\item Sheltered and unsheltered sites show systematic differences in orientation distributions\DIFdelbegin \DIFdel{, influenced by wind}\DIFdelend . Instrument (local perturbations) and atmospheric effects cannot be separated.
...
\begin{abstract}
Knowledge about orientation of falling \DIFdelbegin \DIFdel{snowflakes }\DIFdelend \DIFaddbegin \DIFadd{snow }\DIFaddend is still poorly documented with field measurements despite its importance, for example, in the interpretation of remote sensing data. This study investigates the orientation of \DIFdelbegin \DIFdel{snowflakes }\DIFdelend \DIFaddbegin \DIFadd{snow hydrometeors }\DIFaddend using data from a Multi-Angle Snowflake Camera (MASC). We explore the impact of different observational setups (sheltered vs. unsheltered), wind \DIFdelbegin \DIFdel{conditions}\DIFdelend \DIFaddbegin \DIFadd{speed}\DIFaddend , hydrometeor type, and axis ratio on the orientation distributions. Numerical simulations are used to select the best orientation estimator and to understand the reason behind contrasting results reported in past literature. We find that previously reported non-zero median orientations are likely artifacts due to averaging absolute values of orientations from the three individual cameras. Observed orientations generally follow a symmetrical distribution around 0$^\circ$, with broader distributions observed at unsheltered sites and/or high wind conditions.  Observed distributions may vary significantly from those assumed in previous studies, highlighting the need for further research on \DIFdelbegin \DIFdel{snowflake orientation }\DIFdelend \DIFaddbegin \DIFadd{hydrometeor orientations }\DIFaddend under varying environmental conditions.
\end{abstract}

\section*{Plain Language Summary}
Understanding the orientation of falling \DIFdelbegin \DIFdel{snowflakes }\DIFdelend \DIFaddbegin \DIFadd{snow particles }\DIFaddend is important for interpreting remote sensing data and better understand snowfall micro-physics. This study uses a 3-view camera to capture detailed images of falling \DIFdelbegin \DIFdel{snowflakes }\DIFdelend \DIFaddbegin \DIFadd{particles }\DIFaddend and estimate their orientations. \DIFdelbegin \DIFdel{Snowflakes }\DIFdelend \DIFaddbegin \DIFadd{They }\DIFaddend generally fall with their largest dimension parallel to the ground, with \DIFdelbegin \DIFdel{a standard deviation in }\DIFdelend \DIFaddbegin \DIFadd{typical variability of }\DIFaddend the orientation angle of about 30 degrees. We found that observations at sites exposed to the wind show broader distributions of orientations compared to those in sheltered areas. Additionally, our simulations suggest that some previous studies may have incorrectly reported average orientations because of the way data was processed. These findings help refine our understanding of \DIFdelbegin \DIFdel{snowflake }\DIFdelend \DIFaddbegin \DIFadd{snowfall }\DIFaddend behavior and show the need to better comprehend the physical mechanisms affecting the distribution of \DIFdelbegin \DIFdel{snowflake }\DIFdelend orientations.
...
%DIFDELCMD < %%%
\DIFdelend Ice-phase hydrometeors \DIFdelbegin \DIFdel{, such as individual crystals and aggregates, }\DIFdelend are typically assumed to fall with a preferred orientation. \DIFdelbegin \DIFdel{It is generally accepted that the relatively simple crystals }\DIFdelend \DIFaddbegin \DIFadd{Falling snow particles }\DIFaddend populating frozen clouds are \DIFdelbegin \DIFdel{aligned }\DIFdelend \DIFaddbegin \DIFadd{considered to align preferentially }\DIFaddend in a way that maximizes the area projected in the vertical direction, as \DIFdelbegin \DIFdel{deduced by remote sensing ~\mbox{%DIFAUXCMD
\cite<e.g.,>{Noel_JAMC_2005}  }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{presented in studies conducting remote sensing retrievals }\DIFaddend and laboratory experiments\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Tinklenberg_JFM_2023,Tinklenberg_JFM_2024}}\hspace{0pt}%DIFAUXCMD
. 
This assumption }\DIFdelend \DIFaddbegin \DIFadd{. Such experiments are documented for planar crystals~\mbox{%DIFAUXCMD
\cite{Noel_JAMC_2005,Matrosov_JAS_2005,Tinklenberg_JFM_2024} }\hspace{0pt}%DIFAUXCMD
as well as for more complex and larger snowflakes and rimed hydrometeors~\mbox{%DIFAUXCMD
\cite{Kennedy_JAMC_2011, Ryzhkov_JAMC_2011, Koebschall_EF_2023}}\hspace{0pt}%DIFAUXCMD
, while less is documented for other particle types such as needles, columns or rosettes. 
}

\DIFadd{The preferential orientation of falling hydrometeors }\DIFaddend is exploited by dual-polarization weather radars to retrieve information about the microphysical processes occurring in precipitation and to identify which type of hydrometeors are present and where. \DIFdelbegin \DIFdel{Except for relatively rare situations, like storm electrification \mbox{%DIFAUXCMD
\cite{Krehbiel_MAP_1996,Pineda_JGRA_2019}}\hspace{0pt}%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{This is relevant for }\DIFaddend the interpretation of \DIFdelbegin \DIFdel{radar meteorology relies on the assumption that anisotropic ice particles tend to fall with their major dimension oriented horizontally.
}\DIFdelend \DIFaddbegin \DIFadd{ground-based polarimetric radar data as well as for exploiting satellite-based observations and their assimilation, as done for example by the European Centre for Medium-Range Weather Forecasts ECMWF~\mbox{%DIFAUXCMD
\cite<e.g.,>{Eyre_QJRMS_2022}}\hspace{0pt}%DIFAUXCMD
. For example, recent studies showed that considering hydrometeor orientation in the assimilation of high-frequency radiometry may improve forecast in areas of cloud and precipitation~\mbox{%DIFAUXCMD
\cite{Barlakas_AMT_2021} }\hspace{0pt}%DIFAUXCMD
and produce significant improvements in path-integrated quantities such as the Ice Water Path~\mbox{%DIFAUXCMD
\cite{Kaur_RS_2022}}\hspace{0pt}%DIFAUXCMD
. 
}

\DIFadd{The orientation and falling regime also affects the settling velocity of ice phase hydrometeors, a key parameter linking microphysical properties to other tangible quantities as the precipitation intensity, and that remains to be better understood and documented~\mbox{%DIFAUXCMD
\cite{Heymsfield_JAS_2004}}\hspace{0pt}%DIFAUXCMD
.
%DIF > Except for relatively rare situations, like storm electrification \cite{Krehbiel_MAP_1996,Pineda_JGRA_2019}, the interpretation of radar meteorology relies on the assumption that anisotropic ice particles tend to fall with their major dimension oriented horizontally.
}\DIFaddend The concept of orientation is \DIFdelbegin \DIFdel{thus }\DIFdelend linked to the anisotropic geometry of most \DIFdelbegin \DIFdel{hydrometeors }\DIFdelend \DIFaddbegin \DIFadd{snow particles }\DIFaddend and loses any meaning for isotropic, spherical particles. In the case of \DIFdelbegin \DIFdel{snowflakes, they }\DIFdelend \DIFaddbegin \DIFadd{snowfall, the hydrometeors }\DIFaddend are often approximated by ellipsoids (or ellipses, in case of 2D images as shown in Fig.~\ref{fig:triplet}) and the orientation is thus defined by the angle between the horizontal and the major axis of the ellipsoid.

Considering a population of hydrometeors rather than individual particles, \DIFdelbegin \DIFdel{as is often the case for practical applications, }\DIFdelend the distribution of orientations is often modeled with relatively narrow variability around 0$^\circ$ \DIFaddbegin \DIFadd{values }\DIFaddend (i.e., horizontally oriented)\DIFdelbegin \DIFdel{values}\DIFdelend . Looking at the overview in Table~\ref{table:orientations}, one can observe that modeling studies mostly assume a Gaussian distribution, with standard deviations varying between 3$^\circ$ \cite{Matrosov_JAM_2001} and 60$^\circ$ \cite{Putnam_MWR_2017} depending on the type of hydrometeors. Observational studies \DIFaddbegin \DIFadd{conducted in the field }\DIFaddend \cite{Garrett_GRL_2015} confirm that \DIFdelbegin \DIFdel{differences exist due to hydrometeor type }\DIFdelend \DIFaddbegin \DIFadd{there are differences due to the type of hydrometeor }\DIFaddend and environmental conditions (wind and turbulence). \DIFdelbegin \DIFdel{However, they report as well }\DIFdelend \DIFaddbegin \DIFadd{They also report }\DIFaddend non-zero mean and median orientation values, which is contradictory with what was stated previously. As we will discuss later in this study, this discrepancy \DIFdelbegin \DIFdel{is, in reality, }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend the result of the way multi-view instrumental observations are combined to retrieve an overall estimate of orientation\DIFaddbegin \DIFadd{, at least for more complex three-dimensional snow particles. A recent laboratory study using snow crystals replicas falling in an alcoholic solution~\mbox{%DIFAUXCMD
\cite{Stout_ACP_2024}}\hspace{0pt}%DIFAUXCMD
, however, shows that in quiescent conditions individual planar crystals may have a spiraling fall regime with preferential orientations that depart from 0$^\circ$. A study with similar methods, but focused on larger snow aggregates, instead showed that horizontal orientation is preferred for these hydrometeors~\mbox{%DIFAUXCMD
\cite{Koebschall_EF_2023}}\hspace{0pt}%DIFAUXCMD
. It is to be remarked that those and other studies, in considering objects falling in liquids, use a solid-to-fluid density ratio which is orders lower than in atmospheric precipitation. This may have far-reaching consequences in the settling dynamics~\mbox{%DIFAUXCMD
\cite{Tinklenberg_JFM_2023,Tinklenberg_JFM_2024}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 

The problem of observing the orientations of hydrometeors while they fall \DIFaddbegin \DIFadd{in natural outdoor conditions}\DIFaddend , without generating disturbance induced by the measurement itself, is not trivial. Among the ground-based instruments designed to perform this task, we may cite the two-dimensional video disdrometer (2DVD) \cite{Kruger_JAOT_2002}, the multi-angle snowflake camera (MASC) \cite{Garrett_AMT_2012}, the Precipitation Imaging Package (PIP) \cite{Pettersen_atmosphere_2020}, and the Video In Situ Snowfall Sensor (VISSS) \cite{Maahn_AMT_2024}. These relatively recent instruments differ, for example, in the number of simultaneous viewpoints (one for PIP, three for MASC, two for the others), resolution (on the order of tens of micrometers for MASC and VISSS and hundreds of micrometers for 2DVD and PIP), or instrumental structure (compact for MASC and 2DVD and open-field for PIP and VISSS). These sensors allow for direct microphysical investigations based on pictures of individual hydrometeors. As mentioned above, indirect retrievals of the behavior of large populations of particles within the sampling volume of \DIFdelbegin \DIFdel{remote-sensing }\DIFdelend \DIFaddbegin \DIFadd{remote sensing }\DIFaddend instruments are also commonly \DIFdelbegin \DIFdel{employed}\DIFdelend \DIFaddbegin \DIFadd{used}\DIFaddend , mostly by means of dual-polarization technologies \cite{Matrosov_JAS_2005}.
...
% ---------------
 ...
 \centering
 \DIFdelbeginFL %DIFDELCMD < \begin{tabular}{l p{80mm}} 
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{l p{90mm}} 
 ...
 \hline
   ...
   \hline
    \cite{Kennedy_JAMC_2011}  &   $\overline{\theta} = 0^\circ, \sigma = 15^\circ$ (dendrites)\DIFdelbeginFL %DIFDELCMD < \newline %%%
...
\cite{Melnikov_JAOT_2013} }\hspace{0pt}%DIFAUXCMD
}& \DIFaddFL{$\overline{\theta} = 0^\circ, \sigma  \in [2,20]^\circ$ (ice-phase clouds) }\\
   \hline
    \DIFaddendFL \cite{Ryzhkov_JAMC_2011} & $\overline{\theta} = 0^\circ, \sigma = 40^\circ$ (aggregates \DIFdelbeginFL \DIFdelFL{) }%DIFDELCMD < \newline 
%DIFDELCMD <    %%%
\DIFdelFL{$\overline{\theta} = 0^\circ , \sigma = 40^\circ$ (}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{or }\DIFaddendFL graupel) \\
   ...
   \hline
   \cite{Bukovic_JAMC_2018} &  $\overline{\theta} = 0^\circ, \sigma = 10^\circ$ (dendrites) \DIFdelbeginFL %DIFDELCMD < \newline  %%%
...
$^*$ }&  \DIFaddFL{$\overline{\theta} \in [0,28]^\circ$ (planar crystals, varying Re number)}\\
   ...
   \hline
   \cite{Fitch_JGR_2022}$^*$ & $\theta_{M} = 3^\circ$ \DIFdelbeginFL %DIFDELCMD < \newline
%DIFDELCMD <    %%%
...
To address this, and to select an appropriate orientation estimator, we conducted numerical simulations (results shown in Fig.~\ref{fig:simulations}). These simulations generated ellipsoids with randomly varying geometries, as well as more realistic and geometrically complex aggregates following the same method as \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Leinonen_AMT_2021}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeA{Leinonen_AMT_2021}}\hspace{0pt}%DIFAUXCMD
...
Atmospheric conditions are expected to play a role in shaping orientation distributions\DIFdelbegin \DIFdel{, particularly increasing wind conditions. }\DIFdelend \DIFaddbegin \DIFadd{: atmospheric turbulence is probably a key driver, as well as horizontal winds interacting with the instrument frame~\mbox{%DIFAUXCMD
\cite{Fitch_AMT_2021}}\hspace{0pt}%DIFAUXCMD
. Turbulence has been shown to heavily impact the falling of particles in fluids in general~\mbox{%DIFAUXCMD
\cite{Petersen_JFM_2019,Brandt_ARFM_2022} }\hspace{0pt}%DIFAUXCMD
and of frozen hydrometeors in particular~\mbox{%DIFAUXCMD
\cite{Nemes_JFM_2017,Li_JFM_2021}}\hspace{0pt}%DIFAUXCMD
.
}

 \DIFaddend Figure~\ref{fig:wind-effect} compares the standard deviation of orientation distributions between DFIR and unsheltered sites at different wind speeds. At low wind speeds ($<1$ ms$^{-1}$) and high wind speeds ($>10$ ms$^{-1}$), the curves are close. \DIFdelbegin \DIFdel{However, they }\DIFdelend \DIFaddbegin \DIFadd{They }\DIFaddend diverge by up to 5/6$^\circ$ for intermediate wind speeds, with the DFIR curve showing an increase with wind speed once the wind speed exceeds 4 ms$^{-1}$ and being relatively constant below that threshold. Under near-zero wind conditions, local turbulence induced by the instrument  \DIFdelbegin \DIFdel{geometry is }\DIFdelend \DIFaddbegin \DIFadd{should be }\DIFaddend minimal, making DFIR and unsheltered conditions comparable. One hypothesis could be that until 4 ms$^{-1}$ the DFIR is effective in minimizing local turbulence (DFIR curve constant, unsheltered curve abruptly increasing), while it gradually loses this capacity until "saturating" around 10 ms$^{-1}$ (DFIR curve increasing, unsheltered curve constant). \DIFdelbegin \DIFdel{Making this assumption is however implying that all the observed variations are due to the instrument itself and not a physical phenomenon of interaction of turbulent air and snowflakes}\DIFdelend \DIFaddbegin \DIFadd{This interpretation cannot unfortunately tell us any relevant information about the role of environmental turbulence, but only the likely effect of local turbulence introduced by the instrument}\DIFaddend .  Further research is therefore necessary to clarify these dynamics\DIFaddbegin \DIFadd{, in particular the role of natural turbulence in broadening the orientation distribution}\DIFaddend .
...
At first, estimating orientation as the mean of the orientation of the best-fit ellipsoid from each camera view represents a reasonable approach in terms of minimizing errors and accurately reproducing the distribution of (3D) orientation values. Single-camera estimates exhibit high uncertainty compared to combining estimates from all three views and this should be kept in mind if single-view imagers are used. Previous studies reporting median orientations deviating from 0$^\circ$ in MASC studies \DIFaddbegin \DIFadd{(in natural outdoor conditions) }\DIFaddend are likely erroneous because of the artificially non-zero mode resulting from averaging the \textit{absolute value} of orientation estimates from each camera view. This explanation, supported by numerical simulations in the present study, is simpler than what proposed for example by~\cite{Garrett_GRL_2015} who hypothesized fluttering and local, instrument-induced, turbulence (which has anyway an impact) as possible explanations. \DIFaddbegin \DIFadd{It must be noted that, at least in quiescent conditions and for some types of planar crystals, spiraling falling modes around a non-zero angle have been observed in the laboratory~\mbox{%DIFAUXCMD
\cite{Stout_ACP_2024} }\hspace{0pt}%DIFAUXCMD
but it remains to be seen if this behavior can be observed and isolated also in natural conditions. With this respect, our database is not refined enough to be able to isolate efficiently individual types of planar crystals. 
}

\DIFaddend Field measurements from \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend large dataset demonstrate that orientation values generally exhibit a symmetrical distribution around 0$^\circ$. Observations from unsheltered sites or in very high wind conditions result in broader and flatter orientation distributions, with a less pronounced mode at 0$^\circ$. This is likely influenced by both local disturbances (instrument-induced) and environmental factors, although with the current information we cannot further decipher the two sources.
