...
%DIF ADD Grazioli_JGRA_2024_R1.tex   Sun Dec  1 17:05:21 2024
...
%DIFDELCMD < %%%
\DIFdelend Ice-phase hydrometeors \DIFdelbegin \DIFdel{, such as individual crystals and aggregates, }\DIFdelend are typically assumed to fall with a preferred orientation. It is generally \DIFdelbegin \DIFdel{accepted that the relatively simple crystals }\DIFdelend \DIFaddbegin \DIFadd{considered that falling snow particles }\DIFaddend populating frozen clouds \DIFdelbegin \DIFdel{are aligned }\DIFdelend \DIFaddbegin \DIFadd{align preferentially }\DIFaddend in a way that maximizes the area projected in the vertical direction, as \DIFdelbegin \DIFdel{deduced by remote sensing ~\mbox{%DIFAUXCMD
...
. For example, recent studies showed that considering hydrometeor orientation in the assimilation of high frequency radiometry may improve the  forecast in areas of cloud and precipitation~\mbox{%DIFAUXCMD
\cite{Barlakas_AMT_2021} }\hspace{0pt}%DIFAUXCMD
and produce significant improvements in path-integrated quantities as the Ice Water Path~\mbox{%DIFAUXCMD
\cite{Kaur_RS_2022}}\hspace{0pt}%DIFAUXCMD
. 
}

\DIFadd{The orientation and falling regime also affects the settling velocity of ice phase hydrometers, a key parameter linking microphysical properties to other tangible quantities as the precipitation intensity, and that remains to be better understood and documented~\mbox{%DIFAUXCMD
